...
def cluster_picksim(rmsds, nlocs, nframes, xi=0.05):
    # find different clusters
    X = np.stack([rmsds, nlocs / nframes]).T
    clustering = OPTICS(min_samples=10, xi=xi, min_cluster_size=0.05)
    clustering.fit(X)
...
def find_structures(locs, info, diameter, min_n_locs_per_frame=0.01, xi=0.05):
    ...
    labels = cluster_picksim(rmsds, nlocs, nframes, xi)
