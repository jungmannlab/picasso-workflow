...
def simulate_binary_mask_CSR(
    n_points, mask, area, n_simulations=100, pixelsize=10
    ...
    if isinstance(n_points, int):
        ...
        n_sample = int(
            density * image_area
        )  # density points in mask, scale to whole image

        # create points in units of pixels
        X = np.random.uniform(
            0, mask.shape[0], size=(n_simulations, n_sample, 2)
        )
        ...
        X1 = np.random.uniform(
            0, mask.shape[0], size=(n_simulations, n_sample1, 2)
        )
        X2 = np.random.uniform(
            0, mask.shape[0], size=(n_simulations, n_sample2, 2)
        )
...
def simulate_density_mask_CSR(
    ...
    # image_area = mask.size # units: mask bin size
    if isinstance(n_points, int):
        X = simulate_density_mask_CSR_lower(
            n_points, mask, n_simulations, pixelsize
        )
        return X, X
    else:
        X1 = simulate_density_mask_CSR_lower(
            n_points[0], mask, n_simulations, pixelsize
        )
        X2 = simulate_density_mask_CSR_lower(
...
def get_cell_binary_mask(
    ...
    area = (
        mask_final.astype(float).sum() * upsample**2
    )  # area in nm^2, convert to float to avoid overflow
