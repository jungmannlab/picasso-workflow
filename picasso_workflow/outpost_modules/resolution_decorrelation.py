...
def _get_memory_usage_mb():
    ...
    -------
    tuple
        ``(process_rss_mb, available_mb)``, or ``(-1, -1)`` on error.
    """
    import psutil
    import os
...
def apply_cosine_apodization(image, edge_width=20):
    ...
    edge_width : int, optional
        Width of the edge region to apodize, in pixels. Default is 20.
...
def compute_decorr_single(
    ...
    """Compute the decorrelation curve for a single image.
    ...
    n_r : int, optional
        Number of radial sampling points. Default is 50.
    n_gauss : int, optional
        Number of Gaussian filter strengths. Default is 10.
    apod_edge_width : int, optional
        Edge apodization width in pixels. Default is 20.
    ...
    logger.debug(
        f"    Computing decorrelation for {n_r} radii × {n_gauss} filters..."
    )
...
def extract_resolution_decorr(
    decorr_curve, r_values, pixelsize_render, threshold=0.5
):
    """Extract resolution from a decorrelation curve.
    ...
    threshold : float, optional
        Decorrelation threshold. Default is 0.5.
    ...
    logger.debug(
        f"    Resolution: {resolution:.2f} nm (kc_max: {kc_max:.4f} 1/nm)"
    )
...
def _process_tile_decorr(tile_task):
    """Process a single tile (multiprocessing worker).

    Parameters
    ----------
    tile_task : dict
        Task with keys ``id`` (tile ``(i, j)``), ``bounds``
        (``(x_min, x_max, y_min, y_max)`` in nm), ``locs``, ``pixelsize``,
        ``pixelsize_render``, ``tile_dims``, ``smoothing_sigma``, ``r_min``,
        ``r_max``, ``n_r``, ``n_gauss`` and ``apod_edge_width``.

    Returns
    -------
    result : dict
        On success: ``tile_id``, ``n_locs``, ``decorr_curve``, ``r_values``,
        ``kc_max``, ``resolution`` and ``success=True``. On failure:
        ``tile_id``, ``n_locs``, ``error`` and ``success=False``.
    """
    ...
    try:
        ...
        logger.debug(
            f"    Tile {tile_id}: Rendered {image.shape}, "
        ...
        logger.debug(
            f"    Tile {tile_id}: Resolution = {resolution:.2f} nm, "
...
def compute_decorr_spatial(
    ...
    region_size : float, optional
        Size of each spatial region in micrometers. Default is 10.0 µm.
    min_locs_per_region : int, optional
        Minimum localizations per region. Default is 500.
    ...
    -------
    results : dict
        With ``resolution`` (mean, nm), ``resolution_std``,
        ``resolutions_per_region``, ``decorr_curve_mean``,
        ``decorr_curve_std``, ``r_values``, ``n_regions``,
        ``n_regions_total``, ``n_regions_x`` and ``n_regions_y``.
    """
    from concurrent.futures import ProcessPoolExecutor
    ...
    logger.debug(
        f"Computing image decorrelation with {n_regions_x}×{n_regions_y} regions "
    ...
    logger.debug(
        f"  Processing {len(tile_tasks)} valid regions (skipped "
    ...
    logger.debug(
        f"  Resolution range: {np.nanmin(resolutions):.2f} - "
