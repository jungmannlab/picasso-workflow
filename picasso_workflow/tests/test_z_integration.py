...
class Test_A_PicassoIntegration(unittest.TestCase):
    ...
    def test_04_branch_screen(self):
                    ...
                    "join_modules": [
                        (
                            "dummy_module",
                            {
                                "num_ids": (
                                    "$get_prior_result",
                                    "results, 03_branch, branches, $all, "
                                    "00_identify, num_identifications",
                                ),
                            },
                        ),
                    ],
        ...
        # Some picasso versions raise a DeprecationWarning from the lsq fit
        # path (gausslq); pytest may escalate warnings to errors, which is
        # unrelated to the branch machinery under test here.
        import warnings
        ...
        assert branch_results["labels"] == ["g400", "g800"]
        assert len(branch_results["branches"]) == 2
        for br in branch_results["branches"]:
            assert "00_identify" in br
            assert "01_localize" in br
        # per-branch folders exist under the branch dir
        for tag in ("g400", "g800"):
