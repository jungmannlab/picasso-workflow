...
class TestAnalyse(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ap = analyse.AutoPicasso(
            self.results_folder, analysis_config)
    ...
    def test_02_AutoPicasso_create_sample_movie(self):
        ...
        results = self.ap._create_sample_movie(
            ...
            n_sample=10, min_quantile=0.05, max_quantile=.95,
            fps=1)
        logger.debug(f'results: {results}')

        os.remove(os.path.join(
            self.results_folder, 'samplemov.mp4'))
    ...
    def test_03_AutoPicasso_load_dataset(self, mock_load_movie):
        mock_load_movie.return_value = (
            MockPicassoMovie(), {'info': 'picasso-info'})
